...
def get_energies(grids, d, N_e, Z):
    ...
    if N_e == 1:
        ...
        for i in range(len(d)):
            ...
            print(d[i], P_LDA_Energies[i])

        #return HF_Energies, LDA_Energies, P_HF_Energies, P_LDA_Energies
        return LDA_Energies, P_LDA_Energies


if __name__ == '__main__':
    grids = np.linspace(-10, 10, 801)
    d = np.linspace(0, 6, 61) # Nuclear Distances
    ...
    if N_e == 1:
        ...
        LDA_Energies, P_LDA_Energies = get_energies(grids, d, N_e, Z)
        fig1 = plt.figure(1)
        ax1 = fig1.add_subplot(111)
        ax1.set_xlabel('R')
        ax1.set_ylabel('E0(R)')
    ...
    data = np.column_stack((d, LDA_Energies, P_LDA_Energies))
    np.savetxt('H2_LDA801_e4.txt', data)
