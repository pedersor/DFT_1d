...
class KSInversion:
    ...
    def __init__(self, 
        ...
        self.tar_density = self.f_tar_density[truncation:self.num_f_grids-truncation]
    ...
    def _update_density(self):
        ...
        self.t_density = self.f_density[self.truncation:self.num_f_grids-self.truncation]
    ...
    def _get_tar_density_cost(self):
        ...
        return (s_v.dot((d_n - d_tar)**2))**0.5
    ...
    def _get_f_density_cost(self):
        if self.previous_f_density is not None:
            ...
            return (s_v.dot(abs(d_n - d_p)**2))**0.5
        else:
            return None
    ...
    def _append_exp_cfexp(self, x0, ignored_points):
        ...
        Cr_numerator = d1_h.dot(np.append(self.t_v_XC[-ip-3:-ip-1], [vr, vr, vr]))
        Cr_denominator = d1_h.dot(np.array([0, 0, 0, Ar0-Ar1, Ar0-Ar2])) - self.dx*k*Ar0
        ...
        Cl_denominator = d1_h.dot(np.array([Al0-Al2, Al0-Al1 ,0, 0, 0])) + self.dx*k*Al0
    ...
    def solve_v_XC(self, x0=0, ignored_points=0, f='cfexp'):
        step = 0
        self.step_list = []
        self.cost_list = [[], [], []]
        f_density_cost = None
        ...
        while True:
            ...
            if f_density_cost != None and KE_cost != None:
                if (f_density_cost <= self.tol and KE_cost <= self.tol):
                    break
...
def two_iter_KS_inversion(f_grids, f_v_Z_fn, f_tar_density, num_electrons,
                          n_point_stencil=5, tol=0.000001, t_tol=0.00005):
    ...
    ksi.solve_v_XC(ignored_points=6, f='cfexp')
...
if __name__ == '__main__':  
    ...
    f_v_Z_fn = lambda _:np.load('ks_inversion_npy/Be/potentials.npy')[0]
    f_tar_density = np.load('ks_inversion_npy/Be/densities.npy')[0]
    num_electrons = 4
